...
resource "aws_security_group" "ec2_client_sg" {
  name        = "${local.base_name_2}-ec2-client-sg"
...
resource "aws_security_group" "ec2_web_server_1_sg" {
  name        = "${local.base_name_2}-ec2-web-server-1-sg"
  description = "Security group for ec2 web server 1"
...
resource "aws_security_group" "ec2_web_server_2_sg" {
  name        = "${local.base_name_2}-ec2-web-server-2-sg"
  description = "Security group for ec2 web server 2"
  vpc_id      = module.vpc_2.vpc_id
...
resource "aws_security_group" "vpc_lattice_1_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = [module.vpc_1.vpc_cidr, local.vpc_lattice_cidr]
  }
